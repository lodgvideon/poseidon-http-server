...
| **RateLimit** | `RateLimit(RateLimitConfig)` | Token-bucket limiter; short-circuits with **429** when a bucket is empty. Keyed by `cfg.Key` (single global bucket by default). `Rate` (req/s, default 100) + `Burst` (default `max(1, Rate)`). |
